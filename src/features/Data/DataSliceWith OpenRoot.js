...
export default function Elements() {
    ...
    const RootEdges= useSelector(state=> state.Data.RootEdges)
      ...
      {         
        ...
        RootNode.forEach((shape)=>{
         ...
         CreateRoots( X,id, width, height, name,type,prop,description,trace,depth )  
          ...
          RootEdges.forEach((Relationship)=>{
            let id=Relationship.id;
            let label=Relationship.label;
            let source=Relationship.source;
            let target=Relationship.target;  
    ...
    },[RootNode,filterNodes,ExpandingNodes,Layoutstyle,Edges])
...
function CreateRoots( X,id, width, height, name,type,properties,description,trace,depth ){
    ...
    var attribute =type==='Software Requirements'?attributs.Type_A:
...
function nodeExpantion(id,depth){
  const found = ExpandedNodeids.some(el => el.id === id);
    if(filterNodes.length===0)
    {
    ...
    if(ExpandingNodes.length===0)
      ...
      //for nextedges
      var newrelations= filterEdges.filter(relationship=>relationship.source[0]===id )
      dispatch(setExpandingEdges(newrelations));
          var nextNodesids=[]
          newrelations.map((nextNode,index)=>{
            var target=nextNode.target
        ...
        nextNodesids.map((node,index)=>{
            ...
            var expandingnode=filterNodes.filter(Node=> Node.id===node.target[0] );
            nextNode=[...nextNode,expandingnode[0]]; 
      ...
      //for nextedges
      var newrelations= [...ExpandingEdges,...filterEdges.filter(relationship=>relationship.source[0]===id )]
      dispatch(setExpandingEdges(newrelations));
          var relations= filterEdges.filter(relationship=>  relationship.source[0]===id )
          var nextNodesids=[]
          relations.map((nextNode,index)=>{
            var target=nextNode.target
            nextNodesids=[...nextNodesids,{target} ]
...
function nodeCollapsing(id,depth)
     {    
      console.log(id,depth,'id & depth')
        ...
        console.log(RootsLen,ExpandingNodesLen)
        ...
        if(RootsLen===ExpandingNodesLen){
          ...
          var newEdges=ExpandingEdges.filter((relationship)=>!neighborsID.includes(relationship.target[0])  )
          var newNode=[]
...
console.log(newExpandedNodes,"nodes")
console.log(newExpandedEdges,"edges")
