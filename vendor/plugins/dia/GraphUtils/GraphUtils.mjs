...
// This plugin extends the `dia.Graph` object with additional methods.
...
dia.Graph.prototype.constructTree = function(parent, opt, parentElement, collector) {

    collector = collector || [];

    var children = util.isFunction(opt.children) ? opt.children(parent) : parent[opt.children || 'children'];
...
dia.Graph.prototype.shortestPath = function(source, target, opt) {
    ...
    var previous = Dijkstra(adjacencyList, source.id || source, opt.weight);
